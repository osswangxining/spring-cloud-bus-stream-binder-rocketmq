...
/**
 * Health indicator for RocketMQ.
 *
 * @author Xi Ning Wang
 */
public class RocketMQBinderHealthIndicator implements HealthIndicator {
	...
	public RocketMQBinderHealthIndicator(RocketMQMessageChannelBinder binder,
		...
		Future<Health> future = exec.submit(new Callable<Health>() {
			...
			public Health call() {
				try (Consumer<?, ?> metadataConsumer = consumerFactory.createConsumer()) {
					...
					for (String topic : RocketMQBinderHealthIndicator.this.binder.getTopicsInUse().keySet()) {
						List<PartitionInfo> partitionInfos = metadataConsumer.partitionsFor(topic);
						for (PartitionInfo partitionInfo : partitionInfos) {
							if (RocketMQBinderHealthIndicator.this.binder.getTopicsInUse().get(topic).getPartitionInfos()
